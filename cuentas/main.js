...
// =================================================================
// === INICIO: NUEVO SISTEMA DE PREVISUALIZACIÓN Y EXPORTACIÓN DE PDF ===
// =================================================================

/**
 * Genera el objeto del documento PDF para un informe específico sin descargarlo.
 * @param {string} reportId - El ID del informe.
 * @returns {Promise<jspdf.jsPDF>} Una promesa que se resuelve con el objeto jsPDF.
 */
async function generatePdfDocument(reportId) {
    // Apuntamos al contenedor del *acordeón completo*
    let accordionContent = select(`informe-container-${reportId}`);
    
    // Si el contenido del informe (el gráfico/KPIs) aún no se ha renderizado, lo hacemos ahora.
    if (accordionContent && accordionContent.querySelector('#informe-content-' + reportId).querySelector('.form-label')) {
        await renderInformeDetallado(reportId);
        // Volvemos a seleccionar por si la estructura del DOM ha cambiado
        accordionContent = select(`informe-container-${reportId}`);
    }
    ...
    const informesDisponibles = {
        'extracto_cuenta': { title: 'Extracto de Cuenta' },
        'top_gastos': { title: 'Análisis de Gastos' }, // <-- AÑADIDO
        'crecimiento_patrimonial': { title: 'Crecimiento Patrimonial' }, // <-- AÑADIDO
    };
    const reportTitle = informesDisponibles[reportId]?.title || 'Informe Financiero';
    ...
    doc.text(`Informe generado el ${new Date().toLocaleDateString('es-ES')}`, 14, 28);

    // Buscamos los datos dentro del contenedor de contenido específico
    const reportContentContainer = select(`informe-content-${reportId}`);
    // ¡CORRECCIÓN IMPORTANTE! Añadimos una comprobación por si el contenedor no existe.
    if (!reportContentContainer) {
        console.error(`No se pudo encontrar el contenedor de contenido para el informe: ${reportId}`);
        doc.text("Error: No se pudo generar el contenido del informe.", 14, 40);
        return doc; // Devolvemos el doc con el mensaje de error
    }
    
    const kpiData = reportContentContainer.querySelector(`#kpi-data-${reportId}`);
    const tableData = reportContentContainer.querySelector(`#table-data-${reportId}`);
    const chartCanvas = reportContentContainer.querySelector('canvas');

    let finalY = 40;

    if (kpiData) {
        const kpis = Array.from(kpiData.querySelectorAll('.kpi-item')).map(item => ({
            label: item.querySelector('.kpi-item__label').textContent,
            value: item.querySelector('.kpi-item__value').textContent
        }));
        doc.autoTable({
            startY: finalY,
            head: [['Métrica', 'Valor']],
    ...
    if (chartCanvas) {
        try {
            ...
            doc.text("No se pudo exportar la visualización del gráfico.", 14, finalY + 5);
        }
    }
...
/**
 * Muestra un modal con la previsualización del PDF y un botón para descargarlo.
 * @param {string} reportId - El ID del informe a previsualizar.
 * @param {HTMLButtonElement} btn - El botón que activó la acción.
 */
const showPdfPreviewModal = async (reportId, btn) => {
    setButtonLoading(btn, true, 'Generando...');
    
    try {
        const doc = await generatePdfDocument(reportId);
        
        // --- INICIO DE LA CORRECCIÓN ---
        // Recreamos la lógica para obtener el título aquí, en lugar de intentar leerlo del PDF.
        const informesDisponibles = {
            'resumen_ejecutivo': { title: 'Resumen Ejecutivo' },
            'flujo_caja': { title: 'Análisis de Flujo de Caja' },
            'asignacion_activos': { title: 'Asignación de Activos' },
            'crecimiento_patrimonial': { title: 'Crecimiento Patrimonial' },
            'rendimiento_inversiones': { title: 'Scorecard de Inversiones' }
        };
        const reportTitle = informesDisponibles[reportId]?.title || 'Vista Previa del Informe';
        ...
        const modalHtml = `
            <div style="height: 75vh; display: flex; flex-direction: column;">
                <iframe src="${pdfDataUri}" style="flex-grow: 1; border: none;" title="Previsualización de PDF">
                    <p>Tu navegador no soporta la previsualización. Usa el botón de abajo para descargar.</p>
                </iframe>
            </div>
            <div class="modal__actions">
                <button class="btn btn--secondary" data-action="close-modal" data-modal-id="generic-modal">Cerrar</button>
                <button id="confirm-pdf-download" class="btn btn--primary">
                    <span class="material-icons" style="font-size: 16px;">download</span>
                    <span>Descargar PDF</span>
                </button>
            </div>`;

        showGenericModal(reportTitle, modalHtml);

        // Añadimos el listener para el botón de descarga DENTRO del modal
        const downloadBtn = select('confirm-pdf-download');
        if(downloadBtn) {
            downloadBtn.addEventListener('click', () => {
                doc.save(`Informe_${reportId}_${new Date().toISOString().slice(0,10)}.pdf`);
                hideModal('generic-modal');
        ...
        showToast("No se pudo generar la vista previa.", "danger");
    } finally {
        setButtonLoading(btn, false);
    }
};
...
// =================================================================
// === FIN: NUEVO SISTEMA DE PREVISUALIZACIÓN Y EXPORTACIÓN DE PDF ===
// =================================================================
    ...
    try {
        ...
        let resultadoHtml = `
            <h3 class="card__title" style="display: flex; justify-content: space-between; align-items: center;">
                ...
                <button class="btn btn--secondary" data-action="export-pdf" data-report-id="extracto_cuenta" style="padding: 4px 10px; font-size: 0.75rem;">
        ...
        for (const monthKey of sortedMonthKeys) {
            ...
            // ======================================================================
            // ===               INICIO DE LA LÓGICA 100% CORREGIDA               ===
            // ======================================================================

            // 1. Inicializamos los totales y ordenamos los movimientos para la vista
            let monthIncome = 0;
            let monthExpense = 0;
            movementsForMonth.sort((a, b) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime() || b.id.localeCompare(a.id));

            // 2. Bucle para imprimir PRIMERO los movimientos Y a la vez calcular los totales
            for (const mov of movementsForMonth) {
                // Añadimos la fila del movimiento al HTML
                resultadoHtml += renderInformeCuentaRow(mov, cuentaId, db.cuentas);

                // Calculamos el impacto para el resumen
            ...
            // 3. DESPUÉS de haber impreso todos los movimientos, añadimos la fila de resumen AL FINAL del bloque
            ...
            // ======================================================================
            // ===                FIN DE LA LÓGICA 100% CORREGIDA                 ===
            // ======================================================================
        }
...
const attachEventListeners = () => {
    ...
    if (menu.classList.contains('popover-menu--visible')) {
        ...
        setTimeout(() => {
            ...
            'export-pdf': () => showPdfPreviewModal(reportId, btn),
