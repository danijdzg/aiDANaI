...
/* ========================================================= */
/* === PANTALLA INICIO: GRID FIJO NORD 4 (SIN SCROLL) === */
/* ========================================================= */
const renderPanelPage = async () => {
    ...
    // 2. ESTILOS DE LA REJILLA (GRID)
    // height: calc(100vh - 90px) -> Alto de pantalla MENOS la barra de navegación de abajo
    const containerStyle = `
        height: calc(100vh - 90px);
        padding: 10px 15px;
        display: grid;
        grid-template-rows: auto 1fr 1fr 1.3fr; /* Cabecera, Fila1, Fila2, Patrimonio(Más grande) */
        gap: 10px;
    ...
    // Estilo Títulos (BLANCOS como pediste)
    const titleStyle = `
        font-size: 0.75rem; 
    ...
    // Estilo Valores (Ajuste automático)
    const valStyle = `
    ...
    // 3. ESTRUCTURA HTML (Diseño Bloque a Bloque)
    container.innerHTML = `
        <div style="${containerStyle}">
            
            <div style="display: flex; justify-content: space-between; align-items: center; padding-bottom: 5px;">
                <div style="font-size: 0.9rem; font-weight: 800; color: #fff;">RESUMEN</div>
                <div class="report-filters">
                    <select id="filter-periodo" style="background: rgba(255,255,255,0.1); color: white; border: none; padding: 4px 12px; border-radius: 8px; font-size: 0.8rem;">
            ...
            <div id="custom-date-filters" class="hidden" style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 5px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                <div class="clickable-kpi" data-action="show-kpi-drilldown" data-type="ingresos" 
                     style="${cardBase} border-top: 3px solid #00E676;">
                    <div style="${titleStyle}">Ingresos</div>
                    <div id="kpi-ingresos-value" class="skeleton" style="${valStyle} color: #00E676 !important;">--</div>
                </div>
                <div class="clickable-kpi" data-action="show-kpi-drilldown" data-type="gastos" 
                     style="${cardBase} border-top: 3px solid #FF3D00;">
                    <div style="${titleStyle}">Gastos</div>
                    <div id="kpi-gastos-value" class="skeleton" style="${valStyle} color: #FF3D00 !important;">--</div>
                </div>
            </div>

            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                <div style="${cardBase} border-top: 3px solid #FFFFFF;">
                    <div style="${titleStyle}">Liquidez</div>
                    <div id="kpi-liquidez-value" class="skeleton" style="${valStyle} color: #FFFFFF !important;">--</div>
                </div>
                <div style="${cardBase} border-top: 3px solid #FFD600;">
                    <div style="${titleStyle}">Inversiones</div>
                    <div id="kpi-inversion-total" class="skeleton" style="${valStyle} color: #FFD600 !important;">--</div>
                </div>
            </div>

            <div style="${cardBase} border-top: 4px solid #2979FF;">
                <div style="${titleStyle} font-size: 0.9rem;">Patrimonio Neto Total</div>
                <div id="kpi-patrimonio-neto-value" class="skeleton" 
                     style="font-size: clamp(2.5rem, 8vw, 3.5rem); font-weight: 800; color: #2979FF !important; line-height: 1.1;">
                     --
                </div>
                <div style="margin-top: 8px; font-size: 0.75rem; color: #888;">
                    Ahorro del periodo: <span id="kpi-saldo-neto-value" style="color: #fff; font-weight:bold;">--</span>
                </div>
            </div>
    ...
    // 4. LÓGICA DE FILTROS (Esencial para que funcionen los botones)
    const filterPeriodo = select('filter-periodo');
    ...
    if (filterPeriodo) {
        ...
        filterPeriodo.addEventListener('change', (e) => {
            ...
            localStorage.setItem('dashboard_period_filter', val);
            
            if (val === 'custom') {
                customFiltersDiv.classList.remove('hidden');
            } else {
                customFiltersDiv.classList.add('hidden');
                scheduleDashboardUpdate(); // Actualizar datos
            }
        });
    }

    const dStart = select('filter-fecha-inicio');
    const dEnd = select('filter-fecha-fin');
    if (dStart && dEnd) {
        const checkDates = () => { if (dStart.value && dEnd.value) scheduleDashboardUpdate(); };
        dStart.addEventListener('change', checkDates);
        dEnd.addEventListener('change', checkDates);
    }

    // 5. LLAMADA INICIAL DE DATOS
    scheduleDashboardUpdate();
};
