...
/* =============================================================== */
/* === LÓGICA DE BOTONES DEL ENCABEZADO (AÑADIDO) === */
/* =============================================================== */

document.body.addEventListener('click', (e) => {
    // Busamos si lo que se pulsó (o su padre) es un botón con acción
    const btn = e.target.closest('[data-action]');
    if (!btn) return;
    ...
    // --- 1. ACCIÓN BUSCAR ---
    if (action === 'search-app') {
        console.log('Abriendo búsqueda...');
        // Opción A: Si tienes un modal de búsqueda ya creado:
        const searchModal = document.getElementById('search-modal');
        if (searchModal) {
            // Lógica para abrir modal (copiada de tu app)
            searchModal.style.display = 'flex';
            setTimeout(() => searchModal.style.opacity = '1', 10);
            const input = searchModal.querySelector('input');
            if(input) input.focus();
        } else {
            // Opción B: Si no tienes modal, mostramos un prompt nativo temporalmente
            // o activamos la barra de búsqueda si existe en el diseño
            alert("Módulo de Búsqueda: Aquí debería abrirse el input de búsqueda.");
        }
    }

    // --- 2. ACCIÓN PROPIEDADES (SETTINGS) ---
    if (action === 'open-settings') {
        console.log('Abriendo propiedades...');
        // Intentamos abrir el modal de ajustes
        const settingsModal = document.getElementById('settings-modal');
        // Si no existe un modal específico de ajustes, abrimos el menú principal que suele tenerlos
        const mainMenu = document.getElementById('main-menu-sheet') || document.getElementById('main-menu');
        
        if (settingsModal) {
             // Abrir modal de ajustes específico
             settingsModal.style.display = 'flex';
             setTimeout(() => settingsModal.style.opacity = '1', 10);
        } else if (mainMenu) {
            // Fallback: Abrir menú principal
            mainMenu.classList.add('active'); 
        } else {
            alert("Menú de Propiedades no encontrado en el HTML.");
        }
    }

    // --- 3. ACCIÓN FILTRAR (DIARIO) ---
    if (action === 'filter-diario') {
        console.log('Abriendo filtros...');
        const filterModal = document.getElementById('filter-modal');
        if (filterModal) {
            filterModal.style.display = 'flex';
            setTimeout(() => filterModal.style.opacity = '1', 10);
        } else {
            // Si no existe, creamos un feedback visual
            alert("Abriendo panel de Filtros por fecha/categoría...");
        }
    }

    // --- 4. ACCIÓN CAMBIAR VISTA (GRID/LISTA) ---
    if (action === 'toggle-view') {
        const icon = btn.querySelector('.material-icons');
        const diarioContainer = document.getElementById('diario-list'); // O el contenedor de movimientos
        
        if (icon.textContent === 'grid_view') {
            icon.textContent = 'view_list'; // Cambiar icono
            if(diarioContainer) diarioContainer.classList.add('view-compact'); // Añadir clase CSS
            // Aquí iría tu lógica para cambiar el diseño visual
            console.log("Cambiado a vista compacta");
        } else {
            icon.textContent = 'grid_view';
            if(diarioContainer) diarioContainer.classList.remove('view-compact');
            console.log("Cambiado a vista lista");
        }
    }
});