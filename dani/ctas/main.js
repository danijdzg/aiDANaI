...
const attachEventListeners = () => {
    // 1. Evitar duplicidad de eventos (Limpieza preventiva)
    const newBody = document.body.cloneNode(true);
    // document.body.parentNode.replaceChild(newBody, document.body); // Opcional si el problema persiste

    // 2. Gestor de Haptics (Vibración)
    const enableHaptics = () => {
        userHasInteracted = true;
        document.body.removeEventListener('touchstart', enableHaptics);
        document.body.removeEventListener('click', enableHaptics);
    };
    ...
    // --- GESTOR ÚNICO DE CLICS (Aquí reside la solución) ---
    document.body.addEventListener('click', async (e) => {
        const target = e.target;

        // A. LÓGICA DE MODO PRIVACIDAD (Tus KPIs)
        const kpiPatrimonio = target.closest('#kpi-patrimonio-neto-value') || target.closest('#patrimonio-total-balance');
        if (kpiPatrimonio) {
            document.body.classList.toggle('privacy-mode');
            hapticFeedback('medium');
            localStorage.setItem('privacyMode', document.body.classList.contains('privacy-mode'));
            return;
        }

        // B. CERRAR MENÚS AL TOCAR FUERA
        ...
        // EXTRAEMOS LAS VARIABLES CORRECTAMENTE (Arregla el ReferenceError)
        ...
        // D. MAPA DE ACCIONES (Manteniendo toda tu lógica extensa)
        const actions = {
            ...
            // --- CORRECCIÓN BOTONES DIARIO ---
            'toggle-diario-view': () => {
                hapticFeedback('light');
                if (typeof window.toggleDiarioView === 'function') {
                    window.toggleDiarioView(actionTarget);
                }
            },
            'show-diario-filters': () => {
                hapticFeedback('light');
        ...
        // Ejecutar la acción si existe en el mapa
        if (actions[action]) {
            actions[action](e);
        }
    });

    // 3. Inicializar observadores especializados (Sin duplicar clics)
    initMovementsObserver();
