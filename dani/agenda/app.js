...
let isGoogleSignedIn = false, isAppInitialized = false, isViewSwitching = false;
...
const startMainApp = () => {
    ...
    applyTheme(db.config.theme || 'cyberpunk-night');
    gebi('login-screen').classList.remove('login-view--visible');
    const appRoot = gebi('app-root');
    appRoot.classList.add('visible');
    updateNav(appState.currentView);
    appRoot.querySelector('.view.active')?.classList.remove('active');
    appRoot.querySelector(`#${appState.currentView}View`).classList.add('active');
    renderThemeSelector();
    scheduleAllNotifications();
    if (!db.config.tutorialCompleted && isTutorialReady) {
...
function handleItemFormSubmit(e) {
    ...
    const subtasks = Array.from(qsa('#subtasks-container .subtask-item')).map(el => ({ id: el.dataset.id, description: el.querySelector('input[type="text"]').value, isComplete: el.querySelector('input[type="checkbox"]').checked }));
    ...
    let itemData = {
        ...
        updatedAt: Date.now(),
        isAllDay: gebi('item-type').value === 'agenda' ? gebi('item-all-day').checked : false,
        ...
        reminderMinutes: gebi('item-type').value === 'agenda' && gebi('item-notification-enabled').checked ? parseInt(gebi('item-reminder').value, 10) : 0,
        ...
        googleCalendarEventId: existingItem?.googleCalendarEventId || null,
        parentId: existingItem?.parentId || null
    };

    if (itemData.description.trim() === "") {
        showToast("La descripción no puede estar vacía.", true);
        return;
    }
    ...
    // --- UI Optimista: Guardar localmente y actualizar UI inmediatamente ---
    handleItemSave(itemData, isNewItem);
    closeModal('add-item-modal');
    
    // --- Sincronización en Segundo Plano ---
    if (isGoogleSignedIn && itemData.type === 'agenda') {
        showToast("Sincronizando con Google en segundo plano...", false);
        syncItemWithGoogleInBackground(itemData);
    }
}

async function syncItemWithGoogleInBackground(itemData) {
    try {
        if (itemData.googleCalendarEventId) {
            await actualizarEventoEnGoogle(itemData);
            console.log("Evento actualizado en Google Calendar.");
        } else {
            const googleId = await crearEventoEnGoogle(itemData);
            if (googleId) {
                // Actualizar el item local con el ID de Google y guardar
                const itemIndex = db.items.findIndex(i => i.localId === itemData.localId);
                if (itemIndex > -1) {
                    db.items[itemIndex].googleCalendarEventId = googleId;
                    saveData(); // Guardar el ID de Google en Firebase
                }
            }
            console.log("Evento creado en Google Calendar.");
        }
    } catch (err) {
        console.error("Falló la sincronización en segundo plano con Google:", err);
        showToast("Falló la sincronización con Google. El cambio se guardó localmente.", true);
    }
}
...
function handleItemComplete(localId, isComplete) {
    const itemIndex = db.items.findIndex(i => i.localId === localId);
    if (itemIndex === -1) return;

    const item = { ...db.items[itemIndex] };
    
    // --- UI Optimista: Actualización local inmediata ---
    if (isComplete) {
        showToast(`"${item.description}" completado.`);
        const updatedItem = { ...item, isComplete: true, completedAt: Date.now(), updatedAt: Date.now() };
        if (updatedItem.type === 'task' && updatedItem.subtasks) {
            updatedItem.subtasks.forEach(st => st.isComplete = true);
        }
        ...
        if (item.type === 'agenda' && item.recurrenceRule !== 'none') {
            ...
            if (nextDueDate) {
                const nextEvent = { ...item, localId: uuid(), dueDate: nextDueDate.toISOString(), isComplete: false, completedAt: null, notificationShown: false, createdAt: Date.now(), updatedAt: Date.now(), googleCalendarEventId: null, parentId: item.localId };
                delete nextEvent.googleCalendarEventId;
                db.items.unshift(nextEvent);
                showToast(`Evento reprogramado para la próxima ocurrencia.`);
            }
        }
    } else {
        // Des-completar un item
        if (item.recurrenceRule !== 'none') {
            const childItemIndex = db.items.findIndex(i => i.parentId === localId);
            if (childItemIndex > -1) db.items.splice(childItemIndex, 1);
        }
        const updatedItem = { ...item, isComplete: false, completedAt: null, updatedAt: Date.now() };
        db.items[itemIndex] = updatedItem;
    }
    ...
    // --- Sincronización en Segundo Plano ---
    if (isGoogleSignedIn && item.type === 'agenda' && item.googleCalendarEventId) {
        syncCompletionStateWithGoogleInBackground(item, isComplete);
    }
}

async function syncCompletionStateWithGoogleInBackground(item, isComplete) {
    try {
        if (isComplete) {
            await marcarEventoComoCompletadoEnGoogle(item);
        } else {
            await restaurarEventoEnGoogle(item);
        }
        console.log(`Estado de completado para "${item.description}" sincronizado con Google.`);
    } catch (error) {
        showToast("Error al sincronizar el estado 'completado' con Google.", true);
        console.error("Error in syncCompletionStateWithGoogleInBackground:", error);
    }
}
function handleDeleteItem(localId) {
    triggerHapticFeedback('medium');
    const itemEl = document.querySelector(`.list-item-swipe-container[data-id="${localId}"]`);
    const index = db.items.findIndex(i => i.localId === localId);
    if (index === -1) return;

    const itemToDelete = { ...db.items[index] };

    // --- UI Optimista: Eliminar de la UI y del estado local inmediatamente ---
    const performLocalDelete = () => {
        const finalIndex = db.items.findIndex(i => i.localId === localId);
        if (finalIndex > -1) {
            db.items.splice(finalIndex, 1);
            if (itemToDelete.recurrenceRule !== 'none') {
                 const childIndex = db.items.findIndex(i => i.parentId === localId);
                 if (childIndex > -1) db.items.splice(childIndex, 1);
            }
        }
        saveDataAndSchedule();
        if (!itemEl) renderApp();
    ...
    if (itemEl) {
        ...
        itemEl.addEventListener('animationend', performLocalDelete, { once: true });
    } else {
        performLocalDelete();
    }

    // --- Sincronización en Segundo Plano ---
    if (isGoogleSignedIn && itemToDelete.googleCalendarEventId) {
        deleteFromGoogleInBackground(itemToDelete);
    }
}

async function deleteFromGoogleInBackground(item) {
    try {
        await borrarEventoEnGoogle(item);
        console.log(`Evento "${item.description}" eliminado de Google en segundo plano.`);
    } catch (error) {
        showToast("Error al eliminar el evento de Google Calendar.", true);
        console.error("Error in deleteFromGoogleInBackground:", error);
    }
}
...
function handleQuickAddSubmit(e) {
    ...
    let parsedData = parseNaturalLanguage(input);
    let itemToSave = {
        ...parsedData,
        localId: uuid(),
        isComplete: false,
    ...
    // --- UI Optimista: Guardar localmente y actualizar UI inmediatamente ---
    handleItemSave(itemToSave, true);
    closeModal('quick-add-modal');

    // --- Sincronización en Segundo Plano ---
    if (isGoogleSignedIn && itemToSave.type === 'agenda') {
        showToast("Sincronizando con Google en segundo plano...", false);
        syncItemWithGoogleInBackground(itemToSave);
    }
}
...
function startOnboardingTutorial() {
    if (typeof driver !== 'function') {
        console.error("Se intentó iniciar el tutorial, pero Driver.js no está disponible.");
        return;
    }
    const driverObj = driver.driver({
        showProgress: true,
        steps: [
            { element: '#today-greeting', popover: { title: '¡Bienvenido/a a Agenda aiDANaI!', description: 'Este es tu centro de mando. Permítenos mostrarte rápidamente cómo funciona.' } },
            { element: '#fab-add-item', popover: { title: 'Creación Rápida con IA', description: 'Usa este botón para añadir eventos y tareas usando lenguaje natural, como "Cena con amigos mañana a las 9". ¡La IA se encarga del resto!' } },
            { element: '#todayView', popover: { title: 'Gestos que Ahorran Tiempo', description: 'En cualquier lista, desliza un ítem a la DERECHA para completarlo, y a la IZQUIERDA para eliminarlo. ¡Así de fácil!' } },
            { element: '.bottom-nav', popover: { title: 'Navega por tu Agenda', description: 'Usa estos botones para cambiar entre la vista de Hoy, la Agenda completa, tus listas de Tareas y más.', side: "top", align: 'start' } }
        ],
        onDestroyStarted: async () => {
            if (!driverObj.isActivated()) {
                ...
                try {
                    ...
                    showToast("Error al guardar el estado del tutorial.", true);
                }
            }
        }
    });
...
// --- SECCIÓN DE GOOGLE CALENDAR (VERSIÓN FUSIONADA Y MEJORADA) ---

let gapiIniciado = false;
let gisIniciado = false;
let tokenClient;
let isSyncingWithGoogle = false; // Se mantiene para la sincronización completa, no para acciones individuales
...
function initializeGapiClient() {
    gapi.load('client', () => {
        gapi.client.init({
            ...
            gapiIniciado = true;
            checkReadyStateAndEnableButton();
        }).catch(error => {
            console.error("Error initializing GAPI client:", error);
            showToast("No se pudo iniciar la API de Google Calendar.", true);
        });
    });
}
    ...
    tokenClient = google.accounts.oauth2.initTokenClient({
        ...
        callback: async (tokenResponse) => {
            if (tokenResponse && tokenResponse.access_token) {
                ...
                console.error("Respuesta de token inválida:", tokenResponse);
                showToast("Fallo al obtener el permiso de Google.", true);
            }
        },
        error_callback: (error) => {
            console.error("Error en el flujo de autenticación:", error);
            showToast("Ocurrió un error al conectar con Google.", true);
        }
    });
    gisIniciado = true;
    checkReadyStateAndEnableButton();
...
function checkReadyStateAndEnableButton() {
    if (gapiIniciado && gisIniciado) {
        const syncButton = gebi('google-sync-btn');
        if (syncButton) {
            syncButton.disabled = false;
            syncButton.classList.remove('loading');
            console.log("Bibliotecas de Google listas para usarse.");
        }
    }
}

function handleGoogleAuthClick() {
    if (!gisIniciado || !gapiIniciado) {
        showToast("Las bibliotecas de Google aún no están listas.", true);
        return;
    }
...
function updateGoogleSigninStatus(isSignedIn) { 
    ...
    if (isSignedIn) {
        ...
        syncButton.title = 'Conectado. Haz clic para sincronizar manualmente.';
        syncButton.querySelector('.material-icons').textContent = 'cloud_done';
    } else {
        syncButton.classList.remove('connected');
        syncButton.title = 'Sincronizar con Google';
        syncButton.querySelector('.material-icons').textContent = 'sync';
...
async function marcarEventoComoCompletadoEnGoogle(item) {
    if (!isGoogleSignedIn || !item.googleCalendarEventId || item.type !== 'agenda') return;
    
    // Color gris para "completado" en Google Calendar es el ID 8
    const resource = {
    ...
    try {
        ...
        console.log(`Evento "${item.description}" marcado como completado en Google.`);
    } catch (error) {
        console.error('Error marcando evento como completado en Google:', error);
        throw error;
    }
}

async function restaurarEventoEnGoogle(item) {
    if (!isGoogleSignedIn || !item.googleCalendarEventId || item.type !== 'agenda') return;

    const resource = {
        summary: item.description.replace('[✓] ', ''),
        // Restaura el color original o lo deja por defecto si no tenía uno.
        colorId: item.isImportant ? '11' : null 
    };
    ...
    try {
        ...
        console.log(`Evento "${item.description}" restaurado en Google.`);
    } catch (error) {
        console.error('Error restaurando evento en Google:', error);
        throw error;
    }
}
...
function convertGoogleEventToAidanaiItem(googleEvent) {
    ...
    const isCompletedByUs = googleEvent.summary?.startsWith('[✓]') || googleEvent.colorId === '8';
...
async function syncWithGoogleCalendar(forceFullSync = false) {
    if (isSyncingWithGoogle) {
        showToast("Sincronización ya en curso.");
        return;
    }
    ...
    try {
        ...
        if (syncToken) {
            ...
            timeMin.setFullYear(timeMin.getFullYear() - 2); // Ventana de 2 años hacia atrás
            const timeMax = new Date();
            timeMax.setFullYear(timeMax.getFullYear() + 2); // Ventana de 2 años hacia adelante
            ...
            request = gapi.client.calendar.events.list({
                ...
                showDeleted: true,
                singleEvents: true, // Crucial: expande eventos recurrentes en instancias individuales
                maxResults: 2500
            });
        }
        ...
        // Primero, manejamos los eventos eliminados en Google
        const googleDeletedEventIds = new Set(googleEvents.filter(e => e.status === 'cancelled').map(e => e.id));
        const initialItemCount = db.items.length;
        db.items = db.items.filter(item => {
            if (item.googleCalendarEventId && googleDeletedEventIds.has(item.googleCalendarEventId)) {
                console.log(`Evento eliminado desde Google: ${item.description}`);
                return false; // Elimina el item
            }
            return true; // Conserva el item
        });
        if(db.items.length < initialItemCount) changesMade = true;

        // Luego, actualizamos o añadimos eventos desde Google
        for (const googleEvent of googleEvents) {
            if (googleEvent.status === 'cancelled') continue;

            const existingItem = db.items.find(i => i.googleCalendarEventId === googleEvent.id);
            const newItemData = convertGoogleEventToAidanaiItem(googleEvent);
            if (!newItemData) continue;

            if (existingItem) {
                // Actualiza el item existente con los datos de Google, pero conservando el localId
                const index = db.items.findIndex(i => i.localId === existingItem.localId);
                db.items[index] = { ...existingItem, ...newItemData };
            } else {
        ...
        // Finalmente, subimos los eventos que solo existen localmente
        const localOnlyEvents = db.items.filter(item => item.type === 'agenda' && !item.googleCalendarEventId);
            ...
            for (const localEvent of localOnlyEvents) {
                try {
                    ...
                    if (newGoogleId) {
                     ...
                     console.error(`No se pudo subir el evento local "${localEvent.description}" a Google.`, err);
                }
            }
        }
